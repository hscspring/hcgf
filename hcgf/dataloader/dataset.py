...
class GlmMapStyleDataset:
    ...
    def __init__(
        ...
        self.data = data
        assert hasattr(tokenizer, "eos_token_id"), "tokenizer should have a `eos_token_id` attribute"
    ...
    def __getitem__(self, index: int) -> DataItem:
        ...
        src_ids = self.tokenizer.encode(
            src, max_length=self.max_seq_len, truncation=True, add_special_tokens=True
        )
        tgt_ids = self.tokenizer.encode(
            tgt, max_length=self.max_seq_len, truncation=True, add_special_tokens=False
        )
